...
Only starting from step three and onward (out of ten steps) is the gait stable, and that is illustrated by the state-space plots looping back on itself.
...
Then, when looking at the position of the hip along time on Figure~\ref{fig::virtual_constraints_hip_position}, we can observe two things:

\begin{itemize}
	\item the stability of the gait as assessed from the state-space plots is confirmed by those plots: the first two steps are not in steady-state, but the subsequent steps are quite periodic,
	\item the hip oscillates vertically, while advancing linearly horizontally.
\end{itemize}
