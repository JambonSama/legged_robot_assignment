...
\begin{align}
	...
	f(p) &= w_1 | \dot{x}_\text{hip} - \dot{\bar{x}}_\text{hip}(p) | + w_2 \text{CMT}(p)
\end{align}

where $\dot{x}_\text{hip}$ is the desired horizontal hip velocity, $\bar{\dot{x}}_\text{hip} (p)$ the simulated average horizontal velocity, and $w_i$ weights.
...
\begin{align*}
	\text{CMT} &= \dfrac{U}{x_\text{hip}(T)-x_\text{hip}(1)} \\
