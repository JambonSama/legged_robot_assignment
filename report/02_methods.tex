...
This design of a controller and its optimization is the third step of control design pipeline. The implementation of three controllers were attempted, with various degrees of success.
...
Thus a choice has to made about which generalized coordinates are controlled directly, and which is free to follow the dynamics of the model.
...
The CMT, Cost of Mechanical Transport is defined through the effort $U$ like so:
...
\begin{itemize}
	\item $\mathbf{q0}$ and $\mathbf{\dot{q0}}$,
	\item $k_{p1}$, $k_{p2}$,
	\item $k_{d1}$, $k_{d2}$,
...
\begin{align*}
	\mathbf{f}_\text{swf} &= \begin{bmatrix}
		k_{\text{p}x_\text{swf}} \left( x_\text{swf t}-x_\text{swf} \right) + k_{\text{d}x_\text{swf}} \left( \dot{x}_\text{swf t}-\dot{x}_\text{swf} \right) \\ k_{\text{p}z_\text{swf}} h \sin q_2 + k_{\text{d}z_\text{swf}} \left( \dot{z}_\text{swf t}-\dot{z}_\text{swf} \right)
	\end{bmatrix} \\
...
\begin{align*}
	...
	&= \mathbf{B}^+ \mathbf{J}_\text{top}^T \mathbf{f}_\text{top} + \mathbf{B}^+ \mathbf{J}_\text{hip}^T \mathbf{f}_\text{hip} + \mathbf{B}^+ \mathbf{J}_\text{swf}^T \mathbf{f}_\text{swf}
\end{align*}
...
\begin{align}
	...
	f(p) &= w_1 | \dot{x}_\text{hip d} - \bar{\dot{x}}_\text{hip}(p) | + w_2 \text{CMT}(p) + w_3 \left( z_\text{hip d} - \bar{z}_\text{hip}(p) \right) + w_4 d + w_5 (f_\text{d}-\bar{f})^2 
\end{align}
...
\begin{itemize}
	...
	\item $\bar{\dot{x}}_\text{hip} (p)$ the simulated average horizontal velocity,
	\item $z_\text{hip d}$ the desired vertical position,
	\item $\bar{z}_\text{hip}$ the simulated average vertical position,
	\item $d$ the total distance travelled by the robot,
	\item $f_\text{d}$ is the desired step frequency,
	\item $\bar{f}$ is the simulated average step frequency,
