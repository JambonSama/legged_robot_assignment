...
dx2 = diff(x2, q1) * dq1 + diff(x2, q2) * dq2 % Note: x2 is function of both q1 and q2
dy2 = diff(y2, q1) * dq1 + diff(y2, q2) * dq2 % Note: y2 is function of both q1 and q2
%% Dynamics:
% *Kinetic and potential energies of m1:*

disp('-------------------------')
disp('Kinetic and potential energies of m1:')
T1 =  1/2*m1*l1^2*dq1^2
V1 =  m1*g*y1
...
disp('-------------------------')
dLddq1_dt = diff(dLddq1, q1) * dq1 + diff(dLddq1, dq1) * ddq1 + diff(dLddq1, q2) * dq2 + diff(dLddq1, dq2) * ddq2 % continue: note that dLddq1 is a function of q1, q2, dq1, dq2
%% 
% This is to calculate the formula for $\frac{d}{dt}(\frac{\partial L}{\partial 
% \dot q_2})$:

dLddq2_dt = diff(dLddq2, q1) * dq1 + diff(dLddq2, dq1) * ddq1 + diff(dLddq2, q2) * dq2 + diff(dLddq2, dq2) * ddq2
%%
disp('-------------------------')
...
Eq2 = simplify(Eq2, 'Steps',50) % simplify Eq2 in 50 steps
%%
disp('-------------------------')
disp('Calculate Mass matrix (M), Corriolis and gravity terms (C and  G):')
Eq = simplify(collect([Eq1; Eq2], [ddq1, ddq2, dq1, dq2]))

G(1, 1) = subs(Eq(1),[dq1,ddq1,dq2,ddq2,g],[0,0,0,0,1])*g % hint: you can use subs function 
G(2, 1) = subs(Eq(2),[dq1,ddq1,dq2,ddq2,g],[0,0,0,0,1])*g

M(1, 1) = subs(Eq(1)-G(1),[dq1,ddq2,dq2,ddq1],[0,0,0,1]) % hint: you can use subs function; don't forget to subtract the G term
M(1, 2) = subs(Eq(1)-G(1),[dq1,ddq1,dq2,ddq2],[0,0,0,1])
M(2, 1) = subs(Eq(2)-G(2),[dq1,ddq2,dq2,ddq1],[0,0,0,1])
M(2, 2) = subs(Eq(2)-G(2),[dq1,ddq1,dq2,ddq2],[0,0,0,1])

C(1, 1) = subs(Eq(1)-G(1),[ddq1,ddq2,dq2,dq1],[0,0,0,dq1])/dq1
C(1, 2) = subs(Eq(1)-G(1),[ddq1,ddq2,dq1,dq2],[0,0,0,dq2])/dq2
C(2, 1) = subs(Eq(2)-G(2),[ddq1,ddq2,dq2,dq1],[0,0,0,dq1])/dq1
C(2, 2) = subs(Eq(2)-G(2),[ddq1,ddq2,dq1,dq2],[0,0,0,dq2])/dq2
M = simplify(M) % simplify M
C = simplify(C) % simplify C
...
D0 = jacobian(T, dq).';
D = jacobian(D0, dq)
simplify(D-M) % The Mass matrix is just the jacobian of the total kinetic matrix with respect to the generalized coordinates.